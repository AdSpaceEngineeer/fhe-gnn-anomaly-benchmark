...
from harness.params import ROOT, DEFAULT_THREADS, KEY_POLICY
...
def parse_args(argv=None):
    ...
    p.add_argument("--artifacts", default=str(ROOT / "artifacts" / "toy-v1"))
...
def main(argv=None):
    ...
    try:
        ...
        for index in range(args.num_runs):
            ...
            run = {"index": index, "stages": times, "verification": checked,
                   ...
                   "storage_bytes": {"public_and_evaluation_keys": public_key_bytes,
                                     ...
                                     "persisted_intermediates": directory_bytes(trial / "server" / "intermediate")},
        ...
        report["comparison_scope"] = "Only identical artifact IDs and manifests; toy results are not the trained baseline"
        write_json(out / "report.json", report)
        print("[harness] %s: %s" % (report["status"], out / "report.json"))
        print("[harness] Output contains client secret keys and plaintext test inputs; publish report.json only.")
